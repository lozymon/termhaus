...
- Rust lives in `src-tauri/`: `cargo check`, `cargo clippy`, and `cargo fmt --check` (CI enforces rustfmt). The crate builds **one binary, three faces**: the GUI app, the `termhaus <cmd>` inter-pane control CLI (ADR-0007), and the `termhaus mcp` MCP server — dispatched on the first arg in `main.rs`. The CLI/MCP faces live in `src-tauri/src/cli.rs` + `mcp.rs` (std + serde_json only), sharing the socket client in `src-tauri/src/control_sock.rs`. *(Consolidated from the old separate `th`/`th-mcp` binaries 2026-06-29, mirroring loom.)*
...
**Inter-pane control bus = Rust socket relay, routing in TS** (ADR-0007). A process *inside* a pane (e.g. a `claude` CLI) can drive other panes via the `termhaus <cmd>` CLI → a unix socket at `$TERMHAUS_SOCK`. Rust is a **pure relay**: it forwards the raw request string to the webview as a `termhaus://pane-cmd` event and writes back whatever the frontend replies via `pane_cmd_reply` — it never parses the protocol. All routing (name→pane resolution, writes through the pane registry, `spawn` layout mutation) stays in TS, per the no-product-logic-in-Rust rule. Each PTY child gets `TERMHAUS_SOCK`/`TERMHAUS_PANE`/`TERMHAUS_BIN` injected and the binary's dir prepended to `PATH` (so `termhaus` and `termhaus mcp` resolve inside the pane). This is an **inbound command channel**, distinct from ADR-0001's opacity rule (which forbids parsing pane *output* — still rejected).

## Key files (all exist — this is the map, not a to-do)

**Rust (`src-tauri/src/`):** `main.rs` (one-binary dispatch: `termhaus <cmd>` → CLI, `termhaus mcp` → MCP, else GUI) · `pty.rs` (`PtyManager`: spawn, reader+coalescing-flush threads, write/resize/kill/retarget, reaping) · `lib.rs` (command handlers + Channel wiring + tray/plugin setup) · `control.rs` (inter-pane bus socket relay, ADR-0007) · `cli.rs` (the `termhaus <cmd>` control CLI) · `mcp.rs` (the `termhaus mcp` server) · `control_sock.rs` (socket client shared by both faces) · `tray.rs` (system tray) · `logs.rs` · `capture.rs` · `workspace.rs` (JSON persistence).
...
Most new work follows an existing pattern — overlay/side panels mirror `SessionLogViewer.tsx`; new shortcuts add an action to `lib/keybindings.ts` and wire both dispatch maps (App global + Terminal); new bus ops extend `ControlRequest` in `protocol.ts`, handle in `paneControl.ts`, and add a face in `cli.rs` (a `termhaus <cmd>` subcommand) and/or `mcp.rs` (a `termhaus mcp` tool).
